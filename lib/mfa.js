...
(function() {
    ...
    Mfa.prototype.authenticate = function (userId, userPin, successCb, errorCb) {
        ...
        self.init(function () {
            self.startAuthentication(userId, userPin, ["oidc"], function (data) {
                self.finishAuthentication(userId, userPin, ["oidc"], data.authOTT, successCb, errorCb);
            }, errorCb);
        }, errorCb);
    };

    Mfa.prototype._getOTP = function (userId, userPin, scope, successCb, errorCb) {
        var self = this;
    ...
    Mfa.prototype.fetchOTP = function (userId, userPin, successCb, errorCb) {
        this._getOTP(userId, userPin, ["otp", "otp-auth"], successCb, errorCb);
    };

    Mfa.prototype.fetchRegistrationCode = function (userId, userPin, successCb, errorCb) {
        this._getOTP(userId, userPin, ["otp", "otp-reg"], successCb, errorCb);
    };
    ...
    Mfa.prototype._getPass1 = function (userId, userPin, scope, X, SEC, callback) {
        ...
        errorCode = self.mpin.CLIENT_1(self.mpin.HASH_TYPE, 0, self._hexToBytes(mpinIdHex), self.rng, X, userPin, self._hexToBytes(tokenHex), SEC, U, UT, self._hexToBytes(0));
        if (errorCode !== 0) {
    ...
    Mfa.prototype._renewSecret = function (userId, userPin, sec1Data, successCb, errorCb) {
        ...
        self.request({ url: sec1Data.cs2url }, function(err, sec2Data) {
            ...
            try {
                csHex = self._addShares(sec1Data.clientSecretShare, sec2Data.clientSecret);
                token = self._calculateMPinToken(sec1Data.mpin_id, userPin, csHex);
    ...
    Mfa.prototype.registerDvs = function (userId, userPin, successCb, errorCb) {
        var self = this;

        self.init(function () {
            self.startAuthentication(userId, userPin, ["dvs", "dvs-reg"], function (data) {
                self.finishAuthentication(userId, userPin, ["dvs", "dvs-reg"], data.authOTT, successCb, errorCb);
            }, errorCb);
        }, errorCb);
    };
    ...
    Mfa.prototype._renewDvsSecret = function (userId, userPin, dvsRegister, successCb, errorCb) {
        ...
        self._getDvsSecret1(keypair, dvsRegister.token, function (cs1Data) {
            self._getDvsSecret2(cs1Data, function (cs2Data) {
                try {
                    self.createSigningIdentity(userId, cs1Data["mpinId"], cs1Data["dtas"], cs1Data["dvsClientSecretShare"], cs2Data["dvsClientSecret"], keypair, userPin);
                } catch (err) {
                    return errorCb(err);
                }
    ...
    Mfa.prototype.createSigningIdentity = function (userId, mpinId, dtas, share1Hex, share2Hex, keypair, pinValue) {
        var self = this,
            csHex,
            token,
            signMpinId;
    ...
    Mfa.prototype.signMessage = function (userId, userPin, message, timestamp, successCb, errorCb) {
        ...
        self.init(function () {
            self.startAuthentication(userId, userPin, ["dvs", "dvs-auth"], function (data) {
                self.finishAuthentication(userId, userPin, ["dvs", "dvs-auth"], data.authOTT, function () {

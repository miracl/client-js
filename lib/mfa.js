...
(function() {
    ...
    Errors.missingRequestURL = { code: "MISSING_REQUEST_URL", description: "Missing URL for request" };
    ...
    Mfa.prototype.startRegistration = function(userId, successCb, errorCb) {
        ...
        } else if (!self.users.suitableFor(userId, "start")) {
            return errorCb(Errors.wrongFlow);
        }

        self._registration(userId, function(err) {
            if (err) {
                return errorCb(err);
            }
            successCb(true);
    ...
    Mfa.prototype.confirmRegistration = function(userId, successCb, errorCb) {
        ...
        if (!userId) {
            return errorCb(Errors.missingUserId);
        } else if (!self.users.suitableFor(userId, "confirm")) {
            return errorCb(Errors.wrongFlow);
        }
    ...
    Users.prototype.suitableFor = function(userId, operation) {
        var self = this, userState;

        userState = self.get(userId, "state");
        ...
        switch (operation) {
            ...
            case "confirm":
                return self.exists(userId) && (userState === self.states.start || userState === self.states.active);

            case "finish":
                return self.exists(userId) && userState === self.states.active;
        }

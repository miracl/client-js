...
(function() {
    ...
    Mfa.prototype.signMessage = function (userId, userPin, message, timestamp, successCb, errorCb) {
        ...
        self.init(function () {
            var url = self.options.settings.dvsAuthURL + "?cid=" + self.options.client.clientId;
            self.request({ url: url, type: "POST", authorization: authHeader, data: reqData }, function(err, data) {
                if (err) {
                    if (err.status === 410) {
                        self.users.write(userId, { state: self.mfa.users.states.revoked });
                        errorCb(new Error("Your identity has been revoked"));
                    } else if (err.status === 401) {
                        errorCb(new Error("Wrong PIN"));
                    } else {
                        errorCb(err);
                    }
                } else {
